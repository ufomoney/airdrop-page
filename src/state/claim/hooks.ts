...
function fetchClaimMapping(): Promise<ClaimAddressMapping> {
  return (
    ...
    (FETCH_CLAIM_MAPPING_PROMISE = fetch(
      `https://raw.githubusercontent.com/ufomoney/mrkl-drop-data-chunks/master/mapping.json`
    )
      .then((res) => res.json())
      .catch((error) => {
...
function fetchClaimFile(key: string): Promise<{ [address: string]: UserClaimData }> {
  return (
    ...
    (FETCH_CLAIM_FILE_PROMISES[key] = fetch(
      `https://raw.githubusercontent.com/ufomoney/mrkl-drop-data-chunks/master/${key}.json`
    )
      .then((res) => res.json())
      .catch((error) => {
...
export function useUserClaimData(account: string | null | undefined): UserClaimData | null {
  ...
  return account && chainId === 97 ? claimInfo[account] : null
}
